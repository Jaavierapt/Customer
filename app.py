...
def cargar_contactos():
    ...
    try:
        ...
        return pd.DataFrame(columns=["nombre", "email", "estado", "telefono", "Bitacora", "Rol_Contacto", "Valor"])
...
def cargar_facturas():
    """Consulta todos los registros de la tabla 'Facturas' en Supabase."""
    try:
        ...
        if not data:
            return pd.DataFrame(columns=[
                ...
                "Fecha_Emision", "Fecha_Vencimiento", "Fecha_GES", "Fecha_Pago", "Estado", "Requiere_GES"
            ])
        df = pd.DataFrame(data)
        if "Grupo_Servicio" in df.columns and "Grupo Servicio" not in df.columns:
            df = df.rename(columns={"Grupo_Servicio": "Grupo Servicio"})
        return df
    except Exception as e:
        st.error(f"Error al conectar con la tabla Facturas en Supabase: {e}")
        return pd.DataFrame(columns=[
            "Factura", "Empresa", "Planta", "Grupo Servicio", "Servicio", "Monto",
            "Dias_Programados", "Dias_Reales", "Fecha_Cotizacion", "Fecha_OC",
            "Fecha_Emision", "Fecha_Vencimiento", "Fecha_GES", "Fecha_Pago", "Estado", "Requiere_GES"
        ])
...
def obtener_consejo_ia(notas_bitacora):
    return (
        "💡 **Consejo de IA (Entorno Cloud / Supabase):**\n"
...
if check_password():
    ...
    with st.sidebar:
        ...
        if not df.empty:
            st.download_button("📥 Descargar Reporte PDF", data=generar_pdf(df), file_name="CRM_Itelcam.pdf", mime="application/pdf")

        if not df.empty:
            output_resp = io.BytesIO()
            with pd.ExcelWriter(output_resp, engine='openpyxl') as writer:
                df.to_excel(writer, index=False, sheet_name='Ingresos')
            excel_resp_data = output_resp.getvalue()
            st.download_button(
                label="📥 Descargar Respaldo Excel",
                data=excel_resp_data,
                file_name="Ingresos_Respaldo_Itelcam.xlsx",
    ...
    st.title("🚀 Itelcam CRM - Gestión Estratégica (Cloud + Supabase)")
    ...
    with tab1:
        ...
        if not df.empty and 'Semáforo' in df.columns:
        ...
        if busqueda_global and not df.empty:
            ...
            if user_query:
                with st.spinner("Procesando consulta..."):
                    ...
                    if "ingresos" in query_lower or "total" in query_lower or "cuánto" in query_lower:
                        total_2026_val = df[df['Año'] == 2026]['Monto'].sum() if not df.empty else 0
                        total_2025_val = df[df['Año'] == 2025]['Monto'].sum() if not df.empty else 0
                        respuesta_ia = f"📊 **Análisis (Por Fecha de Pago):** Los ingresos reales pagados durante el año 2026 ascienden a ${total_2026_val:,.0f}, comparados con ${total_2025_val:,.0f} en 2025."
                    elif "cliente" in query_lower or "empresa" in query_lower:
                        top_empresa = df[df['Año'] == 2026].groupby('Empresa')['Monto'].sum().idxmax() if not df.empty and not df[df['Año'] == 2026].empty else "N/A"
                        respuesta_ia = f"🏢 **Análisis:** La empresa con mayor aportación de ingresos reales (pagados) durante el 2026 es **{top_empresa}**."
                    else:
                        respuesta_ia = f"🤖 **Respuesta:** He procesado tu consulta ('{user_query}'). Te sugiero revisar el panel de KPIs ejecutivos y el desglose de ingresos reales por planta."
        ...
        total_2026 = df[df['Año'] == 2026]['Monto'].sum() if not df.empty else 0
        total_2025 = df[df['Año'] == 2025]['Monto'].sum() if not df.empty else 0
        variacion = ((total_2026 - total_2025) / total_2025 * 100) if total_2025 != 0 else 0
        ticket_promedio = df[df['Año'] == 2026]['Monto'].mean() if not df.empty and not df[df['Año'] == 2026].empty else 0
        top_cliente = df[df['Año'] == 2026].groupby('Empresa')['Monto'].sum().idxmax() if not df.empty and not df[df['Año'] == 2026].empty else "N/A"
        ...
        with container_filtros:
            ...
            if not df.empty:
                ...
                if not df_planta_estacional.empty:
                    ...
                    st.warning(f"No hay registros suficientes para la planta {planta_sel}.")
    ...
    with tab4:
        st.header("➕ Gestión de Facturas y Ciclo de Pago (Supabase)")
        ...
        with st.expander("➕ Crear Nueva Factura / Registro de Ingreso"):
            with st.form("form_nueva_factura"):
                ...
                with fc2:
                    ...
                    n_f_pago = st.date_input("Fecha de Pago", value=datetime.now()) if n_estado_pago == "Pagado" else None
                ...
                if st.form_submit_button("💾 Guardar Factura en Base de Datos"):
                    if n_factura.strip() != "" and n_empresa_ins.strip() != "":
                        try:
                            nuevo_registro = {
                                ...
                                "Fecha_Pago": str(n_f_pago) if (n_estado_pago == "Pagado" and n_f_pago) else None,
                                "Estado": n_estado_pago,
                                "Requiere_GES": n_req_ges
                            }
                            supabase.table("Facturas").insert(nuevo_registro).execute()
                            st.cache_data.clear()
                            st.success("¡Factura guardada con éxito en Supabase!")
                            st.rerun()
                        except Exception as e:
        ...
        if not df.empty:
            ...
            st.dataframe(df[columnas_esenciales], use_container_width=True, hide_index=True)
    ...
    with tab5:
        ...
        with st.expander("➕ Crear Nuevo Contacto", expanded=True):
            with st.form("form_contacto_unificado"):
                ...
                if st.form_submit_button("💾 Guardar Registro en Supabase"):
                    if nombre and correo and empresa:
                        try:
                            guardar_contacto(nombre, correo, estado, celular)
                            nueva = pd.DataFrame([{
                                "Nombre": nombre, "Empresa": empresa, "Planta": planta,
                                "Correo": correo, "Celular": celular, "Estado": estado,
                                "Valor": valor, "Bitacora": "", "Rol_Contacto": rol
                            }])
